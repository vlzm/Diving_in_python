"""
Инструкция к выполенению скрипта:
	1. Запускаем server.py, таким образом запустится сервер на адресе 127.0.0.1 и порту 8888 (при необходимости можно изменить адрес и порт)
	2. Проверяем наличие файла client.py
	3. Запускаем check.py
